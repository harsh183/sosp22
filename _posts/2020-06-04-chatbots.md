...
To start, make an account and create a new project in Replit. Next, click on **main.py** and type the following on the first line:
```python
...
async def calc(ctx, x, fn, y):
    ...
    if fn == '+':
        ...
        await ctx.send("We only support 4 function function operations")
```
